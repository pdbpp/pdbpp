...
def test_complete_attribute():
    config = ConfigTest()
    compl = pdb.FancyCompleter({'a': 42}, config)
...
def test_complete_attribute_colored():
    config = ConfigTest()
    config.color_complete = True
    config.color_by_type = {type: '31'}
    compl = pdb.FancyCompleter({'a': 42}, config)
    matches = compl.attr_matches('a.__')
    for match in matches:
        if pdb.setcolor('__class__', '31') in match:
            break
    else:
        assert False
    assert ' ' in matches
...
def test_complete_global():
    config = ConfigTest()
    compl = pdb.FancyCompleter({'foobar': 1, 'foobazzz': 2}, config)
